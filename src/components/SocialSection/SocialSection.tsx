...
export function SocialSection() {
  return (
    <section id="social" className="px-2 py-12 md:px-3 md:py-16">
      <div className="mx-auto max-w-layout rounded-block bg-soft px-4 py-10 md:px-8 md:py-12">
        ...
        <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-text px-4 py-2 text-sm font-semibold text-text transition hover:bg-text hover:text-white"
            >
              YouTube
            </a>
            <a
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1.25fr_0.95fr]">
          <article className="rounded-3xl border border-line bg-white p-4 shadow-soft md:p-5">
            ...
            <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">Travel breakdowns, itinerary math, and behind-the-scenes creator flow.</p>
              </div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex rounded-full border border-text px-4 py-2 text-sm font-semibold text-text transition hover:bg-text hover:text-white"
              >
                Watch on YouTube
              </a>
            </div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {youtubeVideos.map((video) => (
                ...
                >
                  ...
                  />
                  <div className="p-3">
                    <h4 className="text-sm font-semibold leading-snug text-text">{video.title}</h4>
                  </div>
                </a>
              ))}
            </div>
          ...
          <article className="rounded-3xl border border-line bg-white p-4 shadow-soft md:p-5">
            ...
            <div className="mt-3 space-y-3">
              {instagramAccounts.map((account) => (
                <div key={account.id} className="rounded-2xl border border-line bg-soft/40 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <h3 className="text-lg font-semibold text-text">{account.handle}</h3>
                      <p className="mt-1 text-xs leading-relaxed text-muted">{account.bio}</p>
                    </div>
                    <a
                      href={account.profileUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="shrink-0 rounded-full border border-text px-3 py-1.5 text-xs font-semibold text-text transition hover:bg-text hover:text-white"
                    >
                      Follow
                    </a>
                  </div>
